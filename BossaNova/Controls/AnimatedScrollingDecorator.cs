...
namespace Tasks.Show.Controls
{
    public class AnimatedScrollDecorator : Decorator
    {
        ...
        private bool IsAnimationEnabled
        {
            get
            {
                if (this.AnimationBehavior == AnimationBehavior.Never)
                    return false;
                if (this.AnimationBehavior == AnimationBehavior.Always)
                    return true;
                return _isDragging;
            }
        }
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            if (child != null)
            {
                //double w = this.CanDragHorizontally ? double.PositiveInfinity : availableSize.Width; //<-- PositiveInfinity causes xaml parser warning in the designer.
                double w = this.CanDragHorizontally ? 1000000D : availableSize.Width;

                //double h = this.CanDragVertically ? double.PositiveInfinity : availableSize.Height; //<-- PositiveInfinity causes xaml parser warning in the designer.
                double h = this.CanDragVertically ? 1000000D : availableSize.Height;
